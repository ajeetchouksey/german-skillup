...
- `workers/auth.ts` — a dedicated Cloudflare Worker (`workers/schema.sql` for its D1 schema) with its own dedicated Google OAuth client (not shared with `ajch_platform`'s — that was the original plan, but a fully separate client turned out cleaner: zero coupling, no risk of ever touching that platform's live login while managing this one). Modeled on `ajch_platform`'s `workers/subscribe.ts` HMAC-SHA256 session-token design, scoped down to Google-only — no GitHub, no mentor/AI proxy, no comments.
